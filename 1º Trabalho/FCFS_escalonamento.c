...
int dgtlen(int n){
  ...
  if(n == 0){   //se so tem um digito que e 0
  ...
  return c;   //retorna o numero de digitos do numero
}
...
void blocked_check(Fila *f, Processo lista[], int instante){
	int i;

	for(i = 0; i < f->capacidade; i++){
		if(instante==lista[i].waitTime);
		inserir(f, &lista[i]);
	}
}
...
void main () {
  ...
  while(feof(file) == 0)   //feof retorna 0 quando ainda nao chegou ao final do ficheiro
  {
    Vetor = (int *) realloc(Vetor, (Linhas + 1) * sizeof(int));
    fscanf(file, "%d", &Vetor[Linhas]);
    Linhas++;  //quantidade de numeros
  }
  ...
  for(int i = Linhas-1; i > 0; i = i-2){
	...
	for(int i = 0; i < Linhas; i++){
		if(dgtlen(Vetor[i])==3){
			...
			for(int j = i + 2; dgtlen(Vetor[j]) != 3; j = j + 2){
					processo[count_processos].burstTime = Vetor[j];

					if(dgtlen(Vetor[j+1]) != 3 ){
						processo[count_processos].waitTime = Vetor[j + 1];
					}
			}
			//processo[count_processos].burstTime = Vetor[i+2];
			//processo[count_processos].waitTime = Vetor[]
			count_processos++;
		}
	}
...
+-
		...
		if(fila.tamanho!=0){
			run(&fila, &fila.processos[fila.primeiro]);//run current process
			printf("                 ");
	 	}else{
			printf("| RUN "); printf("                    ");
		}
